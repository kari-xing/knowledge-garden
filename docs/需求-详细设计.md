...
| LLM 推理 | Ollama（本地，默认）/ OpenAI 兼容 API | 隐私优先 + 可切换，经 Provider 抽象 |
...
- D5：**LLM 与 Embedding 走 Provider 抽象**。`ollama`（默认）与 `openai_compatible` 两类 Provider，嵌入维度可配置（默认 768）。
...
- AC-6：创建笔记后 30s 内（本地 Ollama）标签/摘要/关联全部生成完毕；前端可见处理状态进度。
...
- LLM Provider 配置（Ollama host / API key / 模型名）。
...
| 维度 | 要求 |
|---|---|
| 性能 | 图谱 ≤500 节点 ≥30fps；检索/看板 P95 < 500ms；AI 流水线 P90 < 30s（本地 Ollama） |
...
5. **降级运行**：关闭 Ollama → 笔记/全文搜索/图谱仍可用，AI 功能标记失败并可手动重试。
...
| 阶段 | 时间 | 任务 | 验收门槛 |
|---|---|---|---|
| W1 | D1-2 | 环境搭建：Docker Compose（postgres/chroma/redis/ollama）、FastAPI 骨架、迁移工具 | `docker compose up` 一键可跑 |
...
| Ollama 本地推理慢 | 流水线超时 | 异步流水线 + 小模型（qwen2.5:7b / bge-m3）+ 批量推理 |
